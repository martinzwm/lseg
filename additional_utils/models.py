...
class LSeg_MultiEvalModule(DataParallel):
    ...
    def forward(self, image, label_set=''):
        ...
        batch, _, h, w = image.size() # martin: 1024, 1024
        assert(batch == 1)
        self.nclass = len(label_set)
        stride_rate = 2.0/3.0
        ...
        for scale in self.scales:
            long_size = int(math.ceil(self.base_size * scale)) # martin: 480
            if h > w:
                height = long_size
                ...
                width = long_size
                height = int(1.0 * h * long_size / w + 0.5) # martin: 481, why 0.5?
                short_size = height
            """
            ...
            cur_img = resize_image(image, height, width, **self.module._up_kwargs) # martin: 480 x 480
            print(type(image), type(cur_img))
            print(image.shape, cur_img.shape)
